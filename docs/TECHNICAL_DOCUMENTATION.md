...
| 数据库 | SQLite、better-sqlite3 | 天气、能耗占比、发电量和 PLC 配置本地存储 |
...
| `/plc-test` | PLC 连接测试 | 测试连接、持久化管理连接参数和读取点位 |
...
| 字段 | 类型 | 说明 |
| --- | --- | --- |
| `key` | TEXT PRIMARY KEY | 种子、版本或应用配置标识 |
| `value` | TEXT | 标识值、写入时间或版本化 JSON 配置 |

PLC 页面保存的连接参数和点位以 `plc_configuration_v1` 为键写入此表，内容包含配置版本、IP、端口、站号、超时、点位列表和更新时间。
...
| `plc.updatePoints(points)` | `plc:update-points` | 替换并持久化读取点位（兼容接口） |
| `plc.saveConfig(input)` | `plc:save-config` | 保存并应用连接参数和读取点位 |
| `plc.resetConfig()` | `plc:reset-config` | 删除持久化配置并恢复环境变量或内置默认值 |
...
| `externalApp.launch()` | `external-app:launch` | 已运行时切换到原窗口，否则启动软件 |
...
连接测试页中的“测试读取”只使用临时参数，不修改运行配置。“保存并应用”会将 IP、端口、站号、超时和完整点位列表写入 SQLite，并立即替换当前运行配置；应用重启时优先加载该持久化配置。“恢复默认”会删除持久化配置，重新使用环境变量或内置默认值。
...
| `PLC_POLL_INTERVAL_MS` | `3000` | 轮询间隔 |
...
- macOS 可选择文件或应用目录；
- 保存前检查目标路径是否存在；
- Windows 启动前按可执行文件路径检查进程；已运行时恢复并切换到原窗口，否则使用 Electron `shell.openPath()` 启动；
- Windows 快捷方式会先解析其目标程序；无法识别目标进程的脚本文件仍使用系统默认方式启动；
...
- 备份：退出应用后复制 `userData` 中的 `weather.sqlite`，其中包含天气数据和 PLC 持久化配置；
...
- PLC 目前只保存一份当前配置，不提供配置历史、导入导出或操作审计；
- S7 原生协议不支持写入；
...
1. 将环境参数和凭据移出源码，使用系统凭据存储或部署配置中心；
2. 为 SQLite 建立显式、可回滚的版本化迁移；
3. 将 IPC 输入统一接入运行时 Schema 校验；
4. 明确“预测”产品口径：若需要 AI，引入可追踪的模型推理链路；
5. 增加结构化日志、日志轮转、设备告警和操作审计；
6. 补齐跨平台 FFmpeg、许可证、签名、公证和自动更新流程；
7. 建立单元、集成、端到端和硬件在环测试。
